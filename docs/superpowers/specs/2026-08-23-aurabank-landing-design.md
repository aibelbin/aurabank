...
- **Portrait viewports enlarge the artwork and crop it.** Contain-fitting a 16:9 frame into a 390×844 phone leaves the artwork about a quarter of the screen height — a thin strip. The frame's upper area is mostly empty, so portrait scales it up (capped at 1.35×) and lets the top crop, taking it to roughly a third of the screen with figures large enough to read. The cap is what keeps the labelled gauges inside the horizontal crop, which is why the ledger is drawn well inside the frame's width.
