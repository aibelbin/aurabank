...
/**
 * The bank may depend on the shared design system and on nothing else in the
 * monorepo. It must never reach into another app — above all not into the
 * landing page's waitlist database. One owner per database: invited addresses
 * arrive here by a copy-paste from an export command, not by a second reader.
 */
...
describe("bank app boundaries", () => {
  ...
  it("never reads the landing page's waitlist", () => {
    const offenders = files.filter(({ contents }) => /waitlist\.db|apps\/landing/.test(contents));
    expect(offenders.map((file) => file.path)).toEqual([]);
  });
});
